class PopulationNEAT {
  ...
  boolean done() {
    for (int i = 0; i< pop.size(); i++) {
      if (!pop.get(i).dead && !pop.get(i).reachedGoal) {
        return false;
      }
    }
    return true;
  ...
  void setBestPlayer() {
    ...
    //data.setInt("value1", 1); 
    //data.setInt("value2", gen); 
    //data.setFloat("value3", tempBest.fitness);
    //data.setInt("value4", tempBest.step);
    
    //if(tempBest.reachedGoal){
  ...
  void calculateFitness() {
    for (int i =1; i<pop.size(); i++) {
      pop.get(i).calculateFitness(lvl.goals.get(lvl.currentGoalIndex));
    }
  }
  //------------------------------------------------------------------------------------------------------------------------------------------
  ...
  void Google() {
    // Ensure there's at least one player in the population
    if (pop.isEmpty()) {
    ...
    Player BestPlayer = pop.get(0);  // Start with the first player as the best
    for (Player p : pop) {
        if (p.fitness > BestPlayer.fitness) {
            BestPlayer = p;  // Replace if we find a player with higher fitness
        }
    }


    // Create a JSONObject with the best player's data
    JSONObject data = new JSONObject();
    data.setInt("value1", TestNumber); 
    data.setInt("value2", gen);  
    data.setFloat("value3", BestPlayer.fitness);  
    data.setInt("value4", BestPlayer.step);  

    // Check if the best player reached the goal
    if (BestPlayer.reachedGoal) {
        data.setString("value5", "Yes");  
    } else {
        data.setString("value5", "No"); 
    }
